...
from mlmm.baal.data_module import DataModule
...
from mlmm.baal.pytorch_dataset import construct_dataset_from_tensor
from mlmm.baal.get_label import EnergyCalculatorBase

if _BAAL_AVAILABLE:
    # from baal.active.dataset import ActiveLearningDataset
    from mlmm.baal.pytorch_dataset import ActiveLearningDataset
    from baal.active.heuristics import AbstractHeuristic, BALD
    from mlmm.baal.myheuristic import ForceVariance, MyAbstractHeuristic
else:
...
class ActiveLearningDataModule(DataModule):
    ...
    def __init__(
        ...
        labeler: Optional[EnergyCalculatorBase] = None,
        #heuristic: "AbstractHeuristic" = BALD(),
        heuristic: "MyAbstractHeuristic" = ForceVariance(),
        ...
        """
        super().__init__(batch_size=1,mode=mode)
        ...
        if self.labelled._ckpt_path is not None:
            self.initial_num_labels = len(self.labelled._train_input)
    ...
    def label(self, probabilities: List[torch.Tensor] = None, indices=None, sample=None):
        if sample is None:
            ...
            if probabilities is not None:
                ...
                uncertainties = self.heuristic.get_uncertainties(probabilities[:,1:19])

                if isinstance(uncertainties,torch.Tensor):
                    indices = torch.argsort(uncertainties)
                else:
            ...
            value = self._dataset.label(-1,sample=sample)
            a = 3
            return value
