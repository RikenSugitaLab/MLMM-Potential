...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(
        self,
        train_input: Optional[Dataset] = None,
        ...
        if ckpt_path is not None and self._train_input is not None:
            # std_e = self._train_input.std_e
            # mean_e = self._train_input.mean_e
            if 'extfield_st' not in self._train_input.label_data.keys():
                for i in range(len(self._train_input)):
                    tmp = self._train_input[i]
            self.restore_dataset(ckpt_path)
            # self._train_input.mean_e = mean_e
            # self._train_input.std_e = std_e
    ...
    def _train_dataloader(self) -> DataLoader:
        ...
        collate_fn = collate #self._train_dataloader_collate_fn
        shuffle: bool = False
    ...
    def _val_dataloader(self) -> DataLoader:
        ...
        collate_fn = collate #self._val_dataloader_collate_fn
    ...
    def _test_dataloader(self) -> DataLoader:
        ...
        collate_fn = collate # self._test_dataloader_collate_fn
    ...
    def _predict_dataloader(self) -> DataLoader:
        ...
        collate_fn = collate #self._predict_dataloader_collate_fn
        ...
        if self.mode == 'database':
            ...
            if self.sampler is None:
                sampler = None
                shuffle = not isinstance(predict_ds, IterableDataset)
            else:
                sampler = self.sampler(predict_ds)
    ...
    def restore_dataset(self, path):
        self._train_input = self._train_input.restore_dataset(path)
