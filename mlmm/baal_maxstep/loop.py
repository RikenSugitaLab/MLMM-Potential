...
class ActiveLearningLoop(Loop):
    ...
    def connect(self, fit_loop: FitLoop):
        ...
        self.fit_loop.max_epochs = -1 #self.label_epoch_frequency #self.label_epoch_frequency 
        self.fit_loop.max_steps = self.label_epoch_frequency #self.label_epoch_frequency 
    ...
    def advance(self, *args: Any, **kwargs: Any) -> None:
        ...
        if self.mode == "generative":
            device = self._lightning_module.std_e.device
            self._lightning_module.std_e = torch.std(self.trainer.predict_loop.energy).to(device)
            self._lightning_module.mean_e = torch.mean(self.trainer.predict_loop.energy).to(device)
    ...
    def _reset_fitting(self):
        ...
        self.fit_loop.epoch_loop.global_step = 0
