...
class mydataset(Dataset):#DGLDataset):
    def __init__(self,
        ...
        force = np.zeros([self.nc.trajectory.n_frames,self.nc.trajectory.n_atoms,3])
        force[:,self.qmidx] = self.label_data['qm_force'].numpy()
        if "mm_force" in self.label_data:
            force[:,self.mmidx] = self.label_data['mm_force'].numpy()

        self.nc.trajectory.force_array = force
        if 'mm_force' in self.label_data.keys():
            self.label_data['mm_force'] = None
    ...
    def __getitem__(self,idx):
        ...
        if self.label_data is not None:
            ...
            for key in self.label_data:
                ...
                # if self.sr_raidus == 0.0 and self.if_cal[idx]==0:
                    # if key=="T1":
                        # if len(g.ntypes)>1:
