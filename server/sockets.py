...
class Driver(DriverSocket):
    ...
    def initialise(self, A, A2):
        self.sendall(Message("init").encode())
        Z = self.get_data(A,send_signal=False)
        C = self.get_data(A2,send_signal=False,need_dataready=False)
        return  Z, C
    ...
    def get_data(self, A, send_signal=True,need_dataready=True):
    ...
    def send_data(self, A, send_signal=True,change_status=True):
