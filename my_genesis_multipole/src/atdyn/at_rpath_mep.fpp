  ...
  subroutine path_update_string(molecule, rpath, dynvars)
    ...
    if (change_world) then
      call mpi_bcast(mep_coord, rpath%dimension*rpath%nreplica,&
                     mpi_wp_real, 0, mpi_comm_world2, ierror)
    else
      call mpi_bcast(mep_coord, rpath%dimension*rpath%nreplica,&
...
#ifdef HAVE_MPI_GENESIS
    if (change_world) then
      call mpi_bcast(conv, 1, mpi_logical, 0, mpi_comm_world2, ierror)
    else
...
#ifdef HAVE_MPI_GENESIS
      if (change_world) then
        call mpi_bcast(task, 60, mpi_character, 0, mpi_comm_world2, ierror)
        call mpi_bcast(rpath%mep_coord(1,1), dim, mpi_wp_real, &
                       0, mpi_comm_world2, ierror)
      else
...
#ifdef HAVE_MPI_GENESIS
      if (change_world) then
        call mpi_bcast(rpath%task, 60, mpi_character, 0, mpi_comm_world2, ierror)
        call mpi_bcast(rpath%mep_coord(1,1), dim, mpi_wp_real, &
                         0, mpi_comm_world2, ierror)
      else
