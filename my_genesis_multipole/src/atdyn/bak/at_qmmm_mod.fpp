...
!        if (main_rank) write(MsgOut, '(a,f10.6,a,i6)') "Adding charge ", molecule%charge(selatoms(i)%idx(j)), " of MM atom ", selatoms(i)%idx(j)
    ...
    if (qmmm%qmtyp == QMtypML .and. replica_main_rank) then
      ...
      do while(.not. istat)
        inquire(file=trim('file.lock'), exist=istat) 
      end do
...
!              if (main_rank) write(MsgOut, '(a,f10.6,a,i6)') "Adding charge ", molecule%charge(list_bond(j)), " of MM link atom ", list_bond(j)
  ...
  !
  !======1=========2=========3=========4=========5=========6=========7=========8
  subroutine compute_electrostatic_mlmm( molecule, coord, qmmm, energy, mode)
    ! formal arguments
    ...
    real(wp), allocatable :: T0_ij(:,:), T1_ij(:,:,:), T2_ij(:,:,:,:)
    real(wp), allocatable :: T3_ij(:,:,:,:,:), T4_ij(:,:,:,:,:,:)
    real(wp), allocatable :: T0_(:), T1_(:,:), T2_(:,:,:)
    real(wp), allocatable :: T3_(:,:,:,:), T4_(:,:,:,:,:)
    ...
    allocate(mm_force(3,qmmm%mm_natoms))
    allocate(T0_ij(n_atoms, qmmm%mm_natoms))
    allocate(T1_ij(3, n_atoms, qmmm%mm_natoms))
    allocate(T2_ij(3, 3, n_atoms, qmmm%mm_natoms))
    allocate(T3_ij(3, 3, 3, n_atoms, qmmm%mm_natoms))
    allocate(T4_ij(3, 3, 3, 3, n_atoms, qmmm%mm_natoms))
    allocate(T0_(n_atoms))
    allocate(T1_(3, n_atoms))
    allocate(T2_(3, 3, n_atoms))
    allocate(T3_(3, 3, 3, n_atoms))
    allocate(T4_(3, 3, 3, 3, n_atoms))
    ...
    !$omp shared(T0_ij_,T1_ij_,T2_ij_,T3_ij_,T4_ij_,link_coord) &
    !$omp shared(T0_ij,T1_ij,T2_ij,T3_ij,T4_ij) &
    !$omp shared(mmatom_id, qmatom_id, coord, mm_charge, mode, max_order, n_atoms) &
    !$omp shared(T0_grad, T1_grad, T2_grad, T3_grad) &
    ...
    do i = 1, qmmm%mm_natoms
      ...
      do j = 1, n_atoms
        ...
        if(mode == 1) then
          ...
          T0_ij(j,i) = elec_tensor_%T0
          T1_ij(:,j,i) = elec_tensor_%T1
          T2_ij(:,:,j,i) = elec_tensor_%T2
          if (sum(abs(T1_ij(:,j,i))) == 0 .and. mm_charge(imm) /= 0) write(*,*) "error1: T1_ij_ is zero. index: ", imm , jqm,'\t' 
          if (max_order > 1) T3_ij(:,:,:,j,i) = elec_tensor_%T3
          if (max_order > 2) T4_ij(:,:,:,:,j,i) = elec_tensor_%T4

        else
          if (qmmm%qmtyp == QMtypML) then
            if (sum(abs(T1_ij_(:,j,i))) == 0 .and. mm_charge(imm) /= 0) write(*,*) "error2: T1_ij_ is zero. index: ", imm , jqm,'\t' 
    ...
    if (mode==1) then
      ...
      qmmm%T0_ij = T0_ij
      T0_ = sum(T0_ij,dim=2)
      call mpi_allreduce(T0_, qmmm%T0, n_atoms, &
                         mpi_wp_real, mpi_sum,              &
                         mpi_comm_country, ierror)

      qmmm%T1_ij = T1_ij
      T1_ = sum(T1_ij,dim=3)
      call mpi_allreduce(T1_, qmmm%T1, n_atoms*3, &
                         mpi_wp_real, mpi_sum,              &
                         mpi_comm_country, ierror)
!      write(*,*) 'T1_ij after_allreduce',qmmm%T1_ij
      call mpi_allreduce(T1_ij, qmmm%T1_ij, n_atoms*qmmm%mm_natoms*3, &
                         mpi_wp_real, mpi_sum,              &
                         mpi_comm_country, ierror)
      write(*,*) sum(abs(sum(qmmm%T1_ij,dim=3) - qmmm%T1))/dble(3*n_atoms)

      qmmm%T2_ij = T2_ij
      T2_ = sum(T2_ij,dim=4)
      call mpi_allreduce(T2_, qmmm%T2, n_atoms*9, &
                         mpi_wp_real, mpi_sum,              &
                         mpi_comm_country, ierror)
!      write(*,*) 'T2_ij after_allreduce',qmmm%T2_ij

      if (max_order > 1) then
        qmmm%T3_ij = T3_ij
        T3_ = sum(T3_ij,dim=5)
        call mpi_allreduce(T3_, qmmm%T3, n_atoms*27, &
                           mpi_wp_real, mpi_sum,              &
                           mpi_comm_country, ierror)
      end if
      ...
      if (max_order > 2) then
        ...
        T4_ = sum(T4_ij,dim=6)
        !write(*,*) T4_ij
        call mpi_allreduce(T4_, qmmm%T4, n_atoms*81, &
                           mpi_wp_real, mpi_sum,              &
                           mpi_comm_country, ierror)
      end if
    end if

    if (qmmm%qmtyp == QMtypML .and. mode == 2 ) then
      do i = 1, 3
  ...
  deallocate(mm_force)
  deallocate(T0_ij, T1_ij, T2_ij, T3_ij, T4_ij)
  deallocate(T0_, T1_, T2_, T3_, T4_)

  end subroutine compute_electrostatic_mlmm
