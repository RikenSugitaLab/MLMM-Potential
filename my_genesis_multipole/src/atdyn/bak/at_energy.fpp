      ...
      do while (qmmm_error)
        ...
        if (qmmm%qmtyp == QMtypQSimulate) then
          ...
          if (qmmm%qmtyp == QMtypML) then !added by YKL
            call compute_electrostatic_mlmm(molecule, coord, qmmm, energy, 1)
         ...
         ! broadcast QM information to other processes
            ...
            call mpi_bcast(qmmm%qm_force(1,1), qmmm%qm_natoms*3, mpi_wp_real, 0,     &
                           mpi_comm_country, ierror)
            call mpi_bcast(qmmm%T0_grad(1) , qmmm%qm_natoms, mpi_wp_real, 0,       &
                           mpi_comm_country, ierror)
            call mpi_bcast(qmmm%T1_grad(1,1) , qmmm%qm_natoms*3, mpi_wp_real, 0,       &
                           mpi_comm_country, ierror)
            if (qmmm%max_order>1) then
              call mpi_bcast(qmmm%T2_grad(1,1,1) , qmmm%qm_natoms*9, mpi_wp_real, 0,       &
                             mpi_comm_country, ierror)
            endif

            if (qmmm%max_order>2) then
              call mpi_bcast(qmmm%T3_grad(1,1,1,1) , qmmm%qm_natoms*27, mpi_wp_real, 0,       &
                             mpi_comm_country, ierror)
            endif
#endif

            call compute_electrostatic_mlmm(molecule, coord, qmmm, energy, 2)
