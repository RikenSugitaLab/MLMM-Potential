...
!              if (main_rank) write(MsgOut, '(a,f10.6,a,i6)') "Adding charge ", molecule%charge(list_bond(j)), " of MM link atom ", list_bond(j)
  ...
  subroutine compute_electrostatic_mlmm( molecule, coord, qmmm, energy, mode)
    ...
    if (mode==1) then
      ...
      call mpi_allreduce(T0_ij, qmmm%T0_ij, n_atoms*qmmm%mm_natoms, &
                         mpi_wp_real, mpi_sum,              &
                         mpi_comm_country, ierror)
      qmmm%T0 = sum(qmmm%T0_ij,dim=2)

      call mpi_allreduce(T1_ij, qmmm%T1_ij, n_atoms*qmmm%mm_natoms*3, &
                         mpi_wp_real, mpi_sum,              &
                         mpi_comm_country, ierror)
      qmmm%T1 = sum(qmmm%T1_ij,dim=3)
!      write(*,*) 'T1_ij after_allreduce',qmmm%T1_ij

      call mpi_allreduce(T2_ij, qmmm%T2_ij, n_atoms*qmmm%mm_natoms*9, &
                         mpi_wp_real, mpi_sum,              &
                         mpi_comm_country, ierror)
!      write(*,*) 'T2_ij after_allreduce',qmmm%T2_ij
      qmmm%T2 = sum(qmmm%T2_ij,dim=4)

      if (max_order > 1) then
        call mpi_allreduce(T3_ij, qmmm%T3_ij, n_atoms*qmmm%mm_natoms*27, &
                           mpi_wp_real, mpi_sum,              &
                           mpi_comm_country, ierror)
        qmmm%T3 = sum(qmmm%T3_ij,dim=5)
      end if

      if (max_order > 2) then
        !write(*,*) T4_ij
        call mpi_allreduce(T4_ij, qmmm%T4_ij, n_atoms*qmmm%mm_natoms*81, &
                           mpi_wp_real, mpi_sum,              &
                           mpi_comm_country, ierror)
        qmmm%T4 = sum(qmmm%T4_ij,dim=6)
      end if
    end if
