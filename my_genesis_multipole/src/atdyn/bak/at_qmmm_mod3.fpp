...
!              if (main_rank) write(MsgOut, '(a,f10.6,a,i6)') "Adding charge ", molecule%charge(list_bond(j)), " of MM link atom ", list_bond(j)
  ...
  !
  !======1=========2=========3=========4=========5=========6=========7=========8
  subroutine compute_electrostatic_mlmm( molecule, coord, qmmm, energy, mm_force, mode)
    ! formal arguments
...
!    allocate(T4_ij(3, 3, 3, 3, n_atoms, qmmm%mm_natoms))
    ...
    if (mode==1) then
      ...
      !qmmm%T0_ij = T0_ij
      call mpi_allreduce(sum(T0_ij_,dim=2), qmmm%T0, n_atoms, &
                         mpi_wp_real, mpi_sum,              &
                         mpi_comm_country, ierror)

      !qmmm%T1_ij = T1_ij
      call mpi_allreduce(sum(T1_ij_,dim=3), qmmm%T1, n_atoms*3, &
                         mpi_wp_real, mpi_sum,              &
                         mpi_comm_country, ierror)
!      write(*,*) 'T1_ij after_allreduce',qmmm%T1_ij

      !qmmm%T2_ij = T2_ij
      call mpi_allreduce(sum(T2_ij_,dim=4), qmmm%T2, n_atoms*9, &
                         mpi_wp_real, mpi_sum,              &
                         mpi_comm_country, ierror)
!      write(*,*) 'T2_ij after_allreduce',qmmm%T2_ij
      ...
      if (max_order > 1) then
       ! qmmm%T3_ij = T3_ij
        call mpi_allreduce(sum(T3_ij_,dim=5), qmmm%T3, n_atoms*27, &
                           mpi_wp_real, mpi_sum,              &
                           mpi_comm_country, ierror)
      end if
      ...
      if (max_order > 2) then
        !qmmm%T4_ij = T4_ij
        !write(*,*) T4_ij
        call mpi_allreduce(sum(T4_ij_,dim=6), qmmm%T4, n_atoms*81, &
                           mpi_wp_real, mpi_sum,              &
                           mpi_comm_country, ierror)
      end if
    end if
