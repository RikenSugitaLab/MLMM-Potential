...
contains
    ...
    if (enefunc%gamd_use) then
      if (enefunc%gamd%boost_dih) then
        ...
        if (enefunc%gamd%boost_qm) then
          ...
          dynvars%total_energy = dynvars%total_energy + & !added by ykl
                               dynvars%energy%qm_gamd
      end if
    end if
  ...
  subroutine output_dynvars_genesis(output, enefunc, dynvars, ensemble)
    ...
    if (enefunc%gamd_use) then
        ...
        if (enefunc%gamd%boost_qm .or. enefunc%gamd%boost_qm_only) then
          write(category(ifm),frmt) 'QM_GAMD'
          values(ifm) = dynvars%energy%qm_gamd
          ifm = ifm+1
        end if
    end if
