...
!! @authors Yoshinobu Akinaga (YA), Kiyoshi Yagi (KY), Kenta Yamada (KYMD), YaoKun Lei(YKL)
!
...
module at_qmmm_mod
  ...
  use constants_mod
  use at_fsockets_mod, only : open_socket, getnodename !added by YKL
  ...
  type, public :: s_qmmm_info
    ...
    integer               :: port                = 31415 !added by YKL
    integer               :: max_order           = 0 !added by YKL

    character(MaxFilename) :: lockindex          = '0' !added by YKL
  ...
  type, private :: s_elec_tensor_local
    real(wp)              :: T0
    real(wp)              :: T1(3)
    real(wp)              :: T2(3,3)
    real(wp)              :: T3(3,3,3)
    real(wp)              :: T4(3,3,3,3)
  end type s_elec_tensor_local
  ...
  public  :: compute_electrostatic_mlmm! added by YKL
  public  :: compute_qmmm_force ! added by YKL
  private :: compute_electrostatic_tensor! added by YKL
...
!    qmmm_info%qm_exemode='system' !added by YKL
    !write(*,*) qmmm_info%qm_exemode
    ...
    call read_ctrlfile_logical(handle, Section, 'qm_debug',           &
                               qmmm_info%qm_debug)
    call read_ctrlfile_integer(handle, Section, 'port',               & !added by YKL
                               qmmm_info%port)
    !write(*,*) "read max_order"
    call read_ctrlfile_integer(handle, Section, 'max_order',          & !added by YKL
                               qmmm_info%max_order)
    call read_ctrlfile_string(handle, Section,  'lockindex',         & !added by YKL
                               qmmm_info%lockindex)
    !write(*,*) "done"
    !write(*,*) "done"
    call read_ctrlfile_string (handle, Section, 'qmexe',              &
    ...
    if (qmmm_info%qmtyp /= QMtypQSimulate .and. &
        ...
        qmmm_info%qmtyp /= QMtypG09_FRWF  .and. &
        qmmm_info%qmtyp /= QMtypML)     & !added by YKL
      call error_msg('Read_Ctrl_QMMM> QM program is not defined')

    if (qmmm_info%qmtyp /= QMtypML .and. trim(qmmm_info%qmcnt) .eq. '') &
      call error_msg('Read_Ctrl_QMMM> QM template file is not defined')

    if (qmmm_info%qmtyp /= QMtypQSimulate .and. qmmm_info%qmtyp /= QMtypML &
        .and. trim(qmmm_info%qmexe) .eq. '') &
...
!ky      endif
  ...
  subroutine setup_qmmm(qmmm_info, sel_info, boundary, psf, rst, forcefield, molecule, qmmm)
    ...
    integer                       :: temp
    integer                       :: inet, color_replica !added by YKL
    real(wp),         allocatable :: charge_(:) !added by YKL
    integer,          allocatable :: group_list(:), atom_no(:)
    real(wp)                      :: mm_mass
    type(s_selatoms), allocatable :: selatoms(:)
    integer                       :: kalloc_stat, kdealloc_stat
    logical                       :: found_atom, setup_qmmm_error, istat
    character                     :: num*5
    character(MaxFilename)        :: path
    character(MaxFilename)        :: host  !added by YKL
    integer                       :: n_atoms, dof !added by YKL
    ...
    qmmm%port                = qmmm_info%port ! added by YKL
    qmmm%max_order           = qmmm_info%max_order ! added by YKL
    ...
    if (qmmm%qmtyp == QMtypML) then
      allocate(charge_(molecule%num_atoms), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc
      do i = 1, molecule%num_atoms
        charge_(i) = molecule%charge(i)
      end do
    endif
...
!        if (main_rank) write(MsgOut, '(a,f10.6,a,i6)') "Adding charge ", molecule%charge(selatoms(i)%idx(j)), " of MM atom ", selatoms(i)%idx(j)
    ...
    ! MM atom indices
    ! store all the index of mm atom into qmmm%mmatom_id (read code)
    ...
    if (change_world) then !added by YKL
      call mpi_barrier(mpi_comm_world2, i)
    else
    ...
    if (qmmm%qmtyp == QMtypML) then
      ...
      allocate(qmmm%T0_grad(n_atoms), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc
      qmmm%T0_grad = 0.0_wp

      allocate(qmmm%T1_grad(3,n_atoms), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc
      qmmm%T1_grad = 0.0_wp

      allocate(qmmm%T2_grad(3,3,n_atoms), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc
      qmmm%T2_grad = 0.0_wp

      allocate(qmmm%T3_grad(3,3,3,n_atoms), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc
      qmmm%T3_grad = 0.0_wp

      allocate(qmmm%T0(n_atoms), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc
      qmmm%T0 = 0.0_wp
      allocate(qmmm%T0_ij(n_atoms,qmmm%mm_natoms), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc
      qmmm%T0_ij = 0.0_wp
      
      allocate(qmmm%T1(3,n_atoms), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc
      qmmm%T1 = 0.0_wp
      allocate(qmmm%T1_ij(3,n_atoms,qmmm%mm_natoms), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc
      qmmm%T1_ij = 0.0_wp
      allocate(qmmm%T2_ij(3,3,n_atoms,qmmm%mm_natoms), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc
      qmmm%T2_ij = 0.0_wp
      allocate(qmmm%T2(3,3,n_atoms), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc
      qmmm%T2 = 0.0_wp
      allocate(qmmm%temporary(3,qmmm%mm_natoms), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc
      qmmm%temporary = 0.0_wp

      dof = n_atoms*3
      qmmm%smg_size = dof * 2
      qmmm%smg_size = qmmm%smg_size + n_atoms 

      if (qmmm%max_order >= 1) qmmm%smg_size = qmmm%smg_size + n_atoms*9
      if (qmmm%max_order >= 2) qmmm%smg_size = qmmm%smg_size + n_atoms*27
      if (qmmm%max_order >= 3) qmmm%smg_size = qmmm%smg_size + n_atoms*81

      qmmm%rmg_size = n_atoms + 1 + dof
      qmmm%rmg_size = qmmm%rmg_size + n_atoms
      qmmm%rmg_size = qmmm%rmg_size + dof
      if (qmmm%max_order > 1) qmmm%rmg_size = qmmm%rmg_size + n_atoms*9
      if (qmmm%max_order > 2) qmmm%rmg_size = qmmm%rmg_size + n_atoms*27

      allocate(qmmm%rmg(qmmm%rmg_size), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc
      qmmm%rmg = 0.0_wp
      allocate(qmmm%smg(qmmm%smg_size), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc
      qmmm%smg = 0.0_wp

      allocate(qmmm%Z_ML(n_atoms), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc
      do i = 1, n_atoms
        if (i<=qmmm%qm_natoms) then
          qmmm%Z_ML(i) = qmmm%qm_atomic_no(i)
        else
          qmmm%Z_ML(i) = 0
        endif
      end do

      allocate(qmmm%coord_ML(3,n_atoms), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc
      qmmm%coord_ML = 0.0_wp

      if (qmmm%max_order > 1) then
        allocate(qmmm%T3_ij(3,3,3,n_atoms,qmmm%mm_natoms), stat = kalloc_stat)
        if(kalloc_stat /= 0) call error_msg_alloc
        qmmm%T3_ij = 0.0_wp
        allocate(qmmm%T3(3,3,3,n_atoms), stat = kalloc_stat)
        if(kalloc_stat /= 0) call error_msg_alloc
        qmmm%T3 = 0.0_wp
      end if

      if (qmmm%max_order > 2) then
        allocate(qmmm%T4_ij(3,3,3,3,n_atoms,qmmm%mm_natoms), stat = kalloc_stat)
        if(kalloc_stat /= 0) call error_msg_alloc
        qmmm%T4_ij = 0.0_wp
        allocate(qmmm%T4(3,3,3,3,n_atoms), stat = kalloc_stat)
        if(kalloc_stat /= 0) call error_msg_alloc
        qmmm%T4 = 0.0_wp
      end if

      allocate(qmmm%qm_classic_charge(n_atoms), stat = kalloc_stat)
      if(kalloc_stat /= 0) call error_msg_alloc

      do i = 1, n_atoms
        if (i<=qmmm%qm_natoms) then
          qmmm%qm_classic_charge(i) = charge_(qmmm%qmatom_id(i))
        else
          qmmm%qm_classic_charge(i) = 0.0_wp
        endif
      enddo
      deallocate(charge_)
      ...
      call getnodename(node_id)
      !write(*,*) "hostname", node_id

#ifdef HAVE_MPI_GENESIS
      if (change_world) then
        call mpi_comm_split(mpi_comm_world2, node_id, my_world_rank, mpi_comm_node, ierror)
      else
        call mpi_comm_split(mpi_comm_world, node_id, my_world_rank, mpi_comm_node, ierror)
      end if

      call mpi_comm_split(mpi_comm_node, color_replica, my_world_rank, &
                          mpi_comm_ifreplica, ierror)
      call mpi_comm_rank(mpi_comm_ifreplica, port_id, ierror)
#else 
      port_id = 0
#endif
      qmmm%port = qmmm%port + port_id
    end if
      
    if (qmmm%qmtyp == QMtypML .and. replica_main_rank) then
      istat = .false.
      !open(93,file='file.lock',form='unformatted')
      !close (93, status='delete')
      do while(.not. istat)
        inquire(file=trim('file.lock.')//trim(qmmm_info%lockindex), exist=istat) 
      end do
      write(MsgOut,*)"Setup_QMMM> Setup MLMM Driver"
      write(MsgOut,'(a,i10)') ' Driver> connecting to host: '//trim(host)//' on port ', qmmm%port
      call open_socket(qmmm%socket, inet, qmmm%port, host)
    end if
  ...
  subroutine setup_mmlist(molecule, qmmm)
    ! residue index of each mm atom (read code)
    ! formal arguments
    type(s_molecule),        intent(inout) :: molecule
    type(s_qmmm),            intent(inout) :: qmmm
...
!              if (main_rank) write(MsgOut, '(a,f10.6,a,i6)') "Adding charge ", molecule%charge(list_bond(j)), " of MM link atom ", list_bond(j)
  ...
  subroutine rearrange_term_list(ityp, qm_natoms, qmatom_id, &
    ...
    ! rearange bondlist to make the first mm_terms mm-bond and the last qm_terms qm-bond(read code)
  ...
  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    compute_qmmm_force
  !> @brief        compute the terms of taylor expansion of electrostatic operator
  !                and caculate the force applied on mm atoms by analytical eq
  !                the gradient of the energy with respect to the terms of taylor
  !                expansion of electrostatic operator 
  !! @authors      YKL
  !! @param[in]    molecule : information of molecules 
  !! @param[in]    coord: coordinates of all atoms
  !! @param[inout] s_energy: information of energy
  !! @param[inout] qmmm: information of qmmm
  !
  !======1=========2=========3=========4=========5=========6=========7=========8
  subroutine compute_qmmm_force( molecule, qmmm, mm_force)
    ! formal arguments
    type(s_molecule), target, INTENT(IN)    :: molecule
    real(wp),                 INTENT(INOUT) :: mm_force(:,:)
    type(s_qmmm),     target, INTENT(INOUT) :: qmmm

    ! local variables
    integer :: i, j, mode
    integer :: id, my_id, nthread, imm, jqm, n_atoms
    integer,  pointer :: mmatom_id(:), qmatom_id(:), max_order
    real(wp), pointer :: T0_ij_(:,:), T1_ij_(:,:,:), T2_ij_(:,:,:,:), T3_ij_(:,:,:,:,:)
    real(wp), pointer :: T4_ij_(:,:,:,:,:,:)
#ifdef OMP
    integer :: omp_get_thread_num, omp_get_max_threads
#endif

    real(wp), pointer :: T0_grad(:), T1_grad(:,:), T2_grad(:,:,:), T3_grad(:,:,:,:)
    T0_grad => qmmm%T0_grad
    T1_grad => qmmm%T1_grad
    T2_grad => qmmm%T2_grad
    T3_grad => qmmm%T3_grad
    ...
    !$omp parallel  &
    !$omp private(id, my_id, i, j, imm, jqm ) &
    !$omp reduction(+:mm_force) 
    ...
    my_id = my_country_rank * nthread + id

    do i = 1, qmmm%mm_natoms
      imm = mmatom_id(i) 

      if (mod(i-1,nthread*nproc_country) /= my_id) cycle

      do j = 1, n_atoms
        if (qmmm%qmtyp == QMtypML) then
          mm_force(:,i) = mm_force(:,i) - T1_ij_(:,j,i)*T0_grad(j)
          mm_force(:,i) = mm_force(:,i) - matmul(T2_ij_(:,:,j,i),T1_grad(:,j))

          if (max_order > 1) then
            mm_force(:,i) = mm_force(:,i) - sum(sum(spread(T2_grad(:,:,j), & 
                                                      dim=3, ncopies=3)*T3_ij_(:,:,:,j,i), dim = 1), dim = 1)
          end if

          if (max_order > 2) then
            mm_force(:,i) = mm_force(:,i) - sum(sum(sum(spread(T3_grad(:,:,:,j),  &
                                                dim=4, ncopies=3)*T4_ij_(:,:,:,:,j,i), dim = 1), dim = 1), dim = 1)
          end if
...
#ifdef HAVE_MPI_GENESIS
    ! Allreduce QM/MM energy and force

    do i = 1, 3
      call mpi_allreduce(mm_force(i,:), qmmm%mm_force(i,:), qmmm%mm_natoms, &
                         mpi_wp_real,  mpi_sum,               &
                         mpi_comm_country, ierror)
  ...
  end subroutine compute_qmmm_force

  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    compute_electrostatic_mlmm 
  !> @brief        compute the terms of taylor expansion of electrostatic operator
  !                and caculate the force applied on mm atoms by analytical eq
  !                the gradient of the energy with respect to the terms of taylor
  !                expansion of electrostatic operator 
  ...
  !
  !======1=========2=========3=========4=========5=========6=========7=========8
  subroutine compute_electrostatic_mlmm( molecule, coord, qmmm)
    ! formal arguments
    type(s_molecule), target, INTENT(IN)    :: molecule
    real(wp),                 INTENT(IN)    :: coord(:,:)
    type(s_qmmm),     target, INTENT(INOUT) :: qmmm
    ...
    type(s_elec_tensor_local) :: elec_tensor_
    real(wp), pointer :: qm_charge(:), mm_charge(:), link_coord(:,:)
    real(wp)          :: dij(3), rij
    qm_charge => qmmm%qm_charge
    mm_charge => molecule%charge
    link_coord => qmmm%linkatom_coord
    ...
    !$omp parallel  &
    !$omp private(id, my_id, i, j, imm, jqm, dij, rij, elec_tensor_) &
    !$omp shared(T0_ij_,T1_ij_,T2_ij_,T3_ij_,T4_ij_) 
    ...
    do i = 1, qmmm%mm_natoms
      ...
      do j = 1, n_atoms
        ...
        ! compute QM/MM energy and force
        ! compute distance
        cal_time = cal_time + 1
        if (j <= qmmm%qm_natoms) then
          jqm = qmatom_id(j)
          dij(1:3) = coord(1:3,jqm) - coord(1:3,imm)
        else
          jqm = j - qmmm%qm_natoms
          dij(1:3) = link_coord(1:3,jqm) - coord(1:3,imm)
          !write(*,*) 'j:',j
          !write(*,*) 'jqm:',jqm
        endif
        rij = sqrt(dij(1)*dij(1) + dij(2)*dij(2) + dij(3)*dij(3))
        call compute_electrostatic_tensor(rij, dij, mm_charge(imm), elec_tensor_, max_order)

        T0_ij_(j,i) = elec_tensor_%T0
        T1_ij_(:,j,i) = elec_tensor_%T1
        T2_ij_(:,:,j,i) = elec_tensor_%T2
        if (max_order > 1) T3_ij_(:,:,:,j,i) = elec_tensor_%T3
        if (max_order > 2) T4_ij_(:,:,:,:,j,i) = elec_tensor_%T4
    ...
    call mpi_reduce(sum(T0_ij_,dim=2), qmmm%T0, n_atoms, &
                       mpi_wp_real, mpi_sum, 0,          &
                       mpi_comm_country, ierror)

    call mpi_reduce(sum(T1_ij_,dim=3), qmmm%T1, n_atoms*3, &
                       mpi_wp_real, mpi_sum, 0,            &
                       mpi_comm_country, ierror)

    call mpi_reduce(sum(T2_ij_,dim=4), qmmm%T2, n_atoms*9, &
                       mpi_wp_real, mpi_sum, 0,            &
                       mpi_comm_country, ierror)

    if (max_order > 1) then
      call mpi_reduce(sum(T3_ij_,dim=5), qmmm%T3, n_atoms*27, &
                         mpi_wp_real, mpi_sum, 0,             &
                         mpi_comm_country, ierror)
    end if

    if (max_order > 2) then
      call mpi_reduce(sum(T4_ij_,dim=6), qmmm%T4, n_atoms*81, &
                         mpi_wp_real, mpi_sum, 0,             &
                         mpi_comm_country, ierror)
    end if
  ...
  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    compute_electrostatic_tensor
  !> @brief        compute the terms of taylor expansion of electrostatic operator
  !! @authors      YKL
  !! @param[in]    rij_: distance between i(qm) and j(mm) atom pair
  !! @param[in]    vij_: relative vector between i(qm) and j(mm) atom pair
  !! @param[in]    mmcharge : charge of mm atom
  !! @param[inout] elec_tensor: struct used to store the results
  !
  !======1=========2=========3=========4=========5=========6=========7=========8
  subroutine compute_electrostatic_tensor( rij_, vij_, mmcharge, elec_tensor, max_order)
    type(s_elec_tensor_local), target, INTENT(INOUT) :: elec_tensor
    real(wp),                           INTENT(IN)    :: rij_
    real(wp),                           INTENT(IN)    :: vij_(3)
    real(wp),                           INTENT(IN)    :: mmcharge
    integer,                            INTENT(IN)    :: max_order
    integer :: i, j, k, l

    real(wp), pointer  :: T0_, T1_(:), T2_(:,:), T3_(:,:,:), T4_(:,:,:,:)
    real(wp)           :: vij(3), vij2(3,1)
    real(wp)           :: rij
    ...
    rij = rij_/CONV_UNIT_LEN
    vij = vij_/CONV_UNIT_LEN
    ...
    T0_ = mmcharge/rij
    T1_ = mmcharge*vij/(rij**3)
    T2_ = mmcharge*(3*matmul(vij2, transpose(vij2)) - (rij**2)*eye)/(rij**5)

    if (max_order > 1) then
      do i = 1, 3 
        do j = i,3
          do k = j,3
            T3_(i,j,k) = (mmcharge*(15.0*vij(i)*vij(j)*vij(k) - &
                                   (3.0*rij**2)*(vij(i)*eye(j,k) + vij(j)*eye(i,k) + vij(k)*eye(i,j))))/(rij**7)
            T3_(i,k,j) = T3_(i,j,k)
            T3_(k,i,j) = T3_(i,j,k)
            T3_(k,j,i) = T3_(i,j,k)
            T3_(j,k,i) = T3_(i,j,k)
            ...
            if (max_order > 2) then
              do l = k,3
                T4_(i,j,k,l) = (mmcharge*(105.0*vij(i)*vij(j)*vij(k)*vij(l) - &
                                        (15.0*rij**2)*(vij(i)*vij(j)*eye(k,l) + vij(i)*vij(k)*eye(j,l) &
                                                     + vij(i)*vij(l)*eye(j,k) + vij(j)*vij(k)*eye(i,l) &
                                                     + vij(j)*vij(l)*eye(i,k) + vij(k)*vij(l)*eye(i,j)) &
                                        +(3.0*rij**4)*(eye(i,j)*eye(k,l) + eye(i,k)*eye(j,l) + eye(i,l)*eye(j,k))))/(rij**9)
                T4_(i,j,l,k) = T4_(i,j,k,l)
                T4_(i,k,j,l) = T4_(i,j,k,l)
                T4_(i,k,l,j) = T4_(i,j,k,l)
                T4_(i,l,j,k) = T4_(i,j,k,l)
                T4_(i,l,k,j) = T4_(i,j,k,l)

                T4_(j,i,k,l) = T4_(i,j,k,l)
                T4_(j,i,l,k) = T4_(i,j,k,l)
                T4_(j,k,i,l) = T4_(i,j,k,l)
                T4_(j,k,l,i) = T4_(i,j,k,l)
                T4_(j,l,i,k) = T4_(i,j,k,l)
                T4_(j,l,k,i) = T4_(i,j,k,l)

                T4_(k,i,j,l) = T4_(i,j,k,l)
                T4_(k,i,l,j) = T4_(i,j,k,l)
                T4_(k,j,i,l) = T4_(i,j,k,l)
                T4_(k,j,l,i) = T4_(i,j,k,l)
                T4_(k,l,i,j) = T4_(i,j,k,l)
                T4_(k,l,j,i) = T4_(i,j,k,l)

                T4_(l,i,j,k) = T4_(i,j,k,l)
                T4_(l,i,k,j) = T4_(i,j,k,l)
                T4_(l,j,i,k) = T4_(i,j,k,l)
                T4_(l,j,k,i) = T4_(i,j,k,l)
                T4_(l,k,i,j) = T4_(i,j,k,l)
                T4_(l,k,j,i) = T4_(i,j,k,l)
              end do
            end if
          end do
        end do
