...
contains
  ...
  subroutine boost_gamd(enefunc, natom, energy, temporary, force, virial)
    ...
    integer                      :: i, j, k, n
    real(wp)                     :: w_pot_gamd, w_dih_gamd, w_qm_gamd ! added by ykl
    real(wp)                     :: ene_pot, ene_dih, ene_qm !added by ykl
    ...
    if (gamd%boost_dih) then
      ...
      if (gamd%boost_qm) then
        ene_pot = energy%bond          &
                ...
                + energy%solvation     
        ene_dih = energy%dihedral
        ene_qm = energy%qm_ene * CONV_UNIT_ENE
      else
        ene_pot = energy%bond          &
                + energy%angle         &
                + energy%urey_bradley  &
    ...
    if (gamd%gamd_stat) then
      ...
      if (gamd%boost_dih) then
        ...
        ! added by ykl
        if (gamd%boost_qm) then
          call compute_stat_gamd(ene_qm, gamd%ene_qm_max, gamd%ene_qm_min, &
            gamd%ene_qm_ave, gamd%ene_qm_ave2, gamd%ene_qm_dev,       &
            gamd%count_qm)
        end if
    ...
    if (gamd%boost_dih) then
      ...
      if (gamd%boost_qm) then
        call compute_boost_potential_gamd(ene_qm, gamd%k_qm, &
          gamd%ene_qm_th, energy%qm_gamd, w_qm_gamd)
      end if
      ...
      do i = 1, natom
        ...
        if (gamd%boost_qm) then
          force(1:3,i) = force(1:3,i) + (w_qm_gamd - w_pot_gamd) * gamd%f_qm(1:3,i) 
        end if
      ...
      do i = 1, natom
        force(1:3,i) = force(1:3,i) + (w_qm_gamd - 1.0_wp) * gamd%f_qm(1:3,i) 
      end do
    end if
