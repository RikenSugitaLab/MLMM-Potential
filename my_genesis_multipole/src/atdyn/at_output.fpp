...
contains
  ...
  subroutine output_gamd(output, dynvars, enefunc)
    ...
    if (output%gamdout) then
        ...
        if (gamd%boost_qm .or. gamd%boost_qm_only) then
          write(category(ifm),frmt) 'QM_MAX'
          values(ifm) = gamd%ene_qm_max
          ifm = ifm+1

          write(category(ifm),frmt) 'QM_MIN'
          values(ifm) = gamd%ene_qm_min
          ifm = ifm+1

          write(category(ifm),frmt) 'QM_AVE'
          values(ifm) = gamd%ene_qm_ave
          ifm = ifm+1

          write(category(ifm),frmt) 'QM_DEV'
          values(ifm) = gamd%ene_qm_dev
          ifm = ifm+1

          write(category(ifm),frmt) 'QM_TH'
          values(ifm) = gamd%ene_qm_th
          ifm = ifm+1

          write(category(ifm),frmt) 'QM_KC'
          values(ifm) = gamd%k_qm
          ifm = ifm+1

          write(category(ifm),frmt) 'QM_KC0'
          values(ifm) = gamd%k0_qm
          ifm = ifm+1
        end if
